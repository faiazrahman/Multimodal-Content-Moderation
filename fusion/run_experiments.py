...
def eval_mmhs_early_fusion():
    run(f"python -m fusion.run_evaluation --config configs/mmhs_fusion/mmhs__early_fusion__text_image_ocr__mpnet_resnet.yaml --trained_model_version XXX --gpus {GPU_ID}")
    run(f"python -m fusion.run_evaluation --config configs/mmhs_fusion/mmhs__early_fusion__text_image_ocr__roberta_resnet.yaml --trained_model_version XXX --gpus {GPU_ID}")
...
def eval_mmhs_low_rank_fusion():
    run(f"python -m fusion.run_evaluation --config configs/mmhs_fusion/mmhs__low_rank_fusion__text_image_ocr__mpnet_resnet.yaml --trained_model_version XXX --gpus {GPU_ID}")
    run(f"python -m fusion.run_evaluation --config configs/mmhs_fusion/mmhs__low_rank_fusion__text_image_ocr__roberta_resnet.yaml --trained_model_version XXX --gpus {GPU_ID}")
